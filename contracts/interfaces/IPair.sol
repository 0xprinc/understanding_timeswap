...
interface IPair {
    ...
    /// @param asset It represents the token which is being borrowed while putting the collateral.
    /// @param collateral It represents the token being put to the collateral.
    struct Tokens {
        uint128 asset;
        uint128 collateral;
    }

    /// @param bondPrincipal represents the token that the lender receives on the asset to exchange for principal number of asset tokens 
    /// @param bondInterest represents the token that the lender receives to exchange for the interest on asset tokens lent  
    /// @param insurancePrincipal represents the token the lender receives to exchange for collateral token in case the asset is not returned
    /// @param insuranceInterest represents the token the lender receives to exchange for the interest on collateral tokens in case the debt is not paid
    struct Claims {
    ...
    /// @param debt the total asset to be paid including the interest by the borrower
    /// @param collateral the collateral kept by the borrower in the smart contract when borrowing 
    /// @param startBlock timestamp of borrowing transaction
    struct Due {
        uint112 debt;
        uint112 collateral;
        uint32 startBlock;
    ...
    /// @dev it is a property of a pool which contains relevant parameters to define its state
    /// @param reserves represents the count of asset and collateral in the pool
    /// @param feeStored the fees related to the overall liquidity left in pool
    /// @param totalLiquidity the total liquidity supply of a Pool
    /// @param totalClaims total claim tokens minted by the lenders and the liquidity providers
    /// @param totalDebtCreated total amount borrowed by the borrowers in the pool
    /// @param x amount of the assets in the pool that can be borrowed.
    /// @param y the interest amount per second
    /// @param z the insurance claims for lenders and collateral required by borrowers
    struct State {
        Tokens reserves;
        uint256 feeStored;
    ...
    /// @dev defines properties of a pool
    /// @param state It gives the state of the pool.
    /// @param liquidities A mapping from address of owner of collateralized debt to liquidity provided by the address in pool.
    /// @param claims mapping of the addresses of owner of collateralized debt to the claim tokens they have.
    /// @param dues mapping of the owner address of the owner of collateralized debt to dues.
    struct Pool {
        State state;
    ...
    /// @dev Returns the claims of a user in a Pool.
    /// @param maturity The unix timestamp maturity of the Pool.
    /// @param owner The address of the user.
